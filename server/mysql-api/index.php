...
function read_config(): array
{
    ...
    if (!is_file($file)) {
        respond(['ok' => false, 'message' => 'config.php nao encontrado na API MedCore.'], 500);
    }

    $config = require $file;
    if (!is_array($config)) {
        respond(['ok' => false, 'message' => 'config.php invalido.'], 500);
    }
    return $config;
}
...
function install_schema(): void
{
    ...
    if ($schema === false) {
        respond(['ok' => false, 'message' => 'schema.sql nao encontrado.'], 500);
    }
